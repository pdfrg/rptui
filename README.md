...
While Radio Paradise offers the absolute best internet radio stations -- human-curated and completely commercial-free, 
The official clients are heavy and limited, while generic internet radio players lack depth and suffer from buffering.
rptui delivers full audio file playback, beautiful visuals, real offline capability, tight Radio Paradise integration, and user-friendly customization in one fast, dependency-light package.

Stars are appreciated if you enjoy the project.
...
- **Four Smart Layouts**: `--layout large` (default, full dashboard with toggleable bottom views), `medium`, `compact`, and `narrow` (perfect vertical sidebar).
...
- **Comments**: Show song comments.  Loads 20 comments with pagination.
...
- **Auto-Add to Blocklist**: Configurable setting (default = false).  Auto-skip all songs with user rating below configurable threshhold (default <=3).

## Screenshots

**Visualizer**
![Visualizer](assets/rptui-fullscreen-visualizer.png)
...
### Quick Installation (requires last.fm API account for scrobbling, see [DOCUMENTATION.md](DOCUMENTATION.md))

```bash
# Recommended: install via Go
go install github.com/pdfrg/rptui@latest
```
### Build from Source (requires last.fm API account for scrobbling)
...
A pre-built binary for Linux/x86 with last.fm support is downloadable from releases.

## Files
The app follows the [XDG Base Directory Specification](https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html):

| Variable | Default | Purpose |
|----------|---------|---------|
| `$XDG_CONFIG_HOME` | `~/.config/rptui/` | Config file, auth tokens |
| `$XDG_CACHE_HOME` | `~/.cache/rptui/` | Favorites, blocklist, offline cache |
| `$XDG_STATE_HOME` | `~/.local/state/rptui/` | Log file |

On first run, default configuration is saved to `~/.config/rptui/config.toml`.
